...
const translations = {
  en: {
    ...
    accounts: {
      ...
      typeIsa: 'ISA', typeCredit: 'Credit card', typeCash: 'Cash', typePension: 'Pension', typeInvestment: 'Investment',
      ...
      importSuccess: 'Imported {amt} — check it looks right, then Save.',
      importSuccessWithDate: 'Imported {amt} as of {date} — check it looks right, then Save.',
      importFailed: "Couldn't read that file — expected a JSON file with a totalValue figure.",
      balanceAsOf: 'as of {date}',
      gainSinceCost: '+{amt} since cost', lossSinceCost: '-{amt} since cost'
    },
    sync: {
    ...
    help: {
      ...
      savingsGoalsBody: "Set a target amount tied to one or more of your accounts — e.g. an \"ISA deposit target: £5,000\" goal tied to your ISA account, or a house deposit goal combining your ISA and Premium Bonds. There's no separate pot to top up: progress is simply those accounts' combined current balance against the target, so any Transfer or Standing Order that moves money into any of them counts automatically.",
  ...
  es: {
    ...
    accounts: {
      ...
      typeIsa: 'ISA', typeCredit: 'Tarjeta de crédito', typeCash: 'Efectivo', typePension: 'Pensión', typeInvestment: 'Inversión',
      ...
      importSuccess: 'Importado {amt} — comprueba que es correcto y luego pulsa Guardar.',
      importSuccessWithDate: 'Importado {amt} a fecha de {date} — comprueba que es correcto y luego pulsa Guardar.',
      importFailed: 'No se pudo leer ese archivo — se esperaba un archivo JSON con un valor totalValue.',
      balanceAsOf: 'a fecha de {date}',
      gainSinceCost: '+{amt} desde el coste', lossSinceCost: '-{amt} desde el coste'
    },
    sync: {
    ...
    help: {
      ...
      savingsGoalsBody: 'Establece un importe objetivo vinculado a una o varias de tus cuentas — p. ej. una meta "Objetivo de depósito ISA: £5.000" vinculada a tu cuenta ISA, o una meta de entrada para una vivienda que combine tu ISA y tus Premium Bonds. No hay un fondo aparte que rellenar: el progreso es simplemente el saldo actual combinado de esas cuentas frente al objetivo, así que cualquier Transferencia u Orden permanente que mueva dinero a cualquiera de ellas cuenta automáticamente.',
